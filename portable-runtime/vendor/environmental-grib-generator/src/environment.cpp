...
public:
  explicit Workspace(const std::filesystem::path& output, bool keep)
      : keep_(keep) {
    static std::atomic<std::uint64_t> next_workspace{0};
    path_ = output.parent_path().empty() ? std::filesystem::current_path()
                                         : output.parent_path();
    path_ /=
        ".environmental-grib-" + std::to_string(ProcessId()) + "-" +
        std::to_string(next_workspace.fetch_add(1, std::memory_order_relaxed));
...
EnvironmentResult GenerateEnvironment(const EnvironmentRequest& request,
  ...
  if (request.max_concurrent_downloads == 0 ||
      request.max_concurrent_downloads > 8)
    throw ValidationError(
        "maximum concurrent downloads must be between 1 and 8");
  const auto download_budget =
      request.download_budget ? request.download_budget
                              : std::make_shared<DownloadConcurrencyBudget>(
                                    request.max_concurrent_downloads);
  HttpGet unbounded_http_get =
      http_get ? std::move(http_get) : HttpGet(CurlHttpGet);
  http_get = [unbounded_http_get, download_budget](const std::string& url,
                                                   double timeout) {
    auto permit = download_budget->Acquire();
    return unbounded_http_get(url, timeout);
  };
  HttpGetRange http_get_range = [download_budget](
                                    const std::string& url, std::size_t start,
                                    std::size_t end, double timeout) {
    auto permit = download_budget->Acquire();
    return CurlHttpGetRange(url, start, end, timeout);
  };
  CredentialValidator credential_validator =
      [download_budget](const std::string& username,
                        const std::string& password, double timeout) {
        auto permit = download_budget->Acquire();
        return ValidateCopernicusCredentials(username, password, timeout);
      };
  ...
  if (request.extend_forecast) {
    EnvironmentRequest extended = request;
    extended.download_budget = download_budget;
    return GenerateExtendedEnvironment(extended, std::move(unbounded_http_get),
                                       now, std::move(progress));
  }
  ...
  std::optional<std::string> selected_cycle;
  Json::Value diagnostics(Json::objectValue);
  diagnostics["maximum_concurrent_downloads"] =
      Json::UInt64(download_budget->maximum());
  ...
  if (request.parallel_components && request.include_waves && !coupled_gfs &&
    ...
    child.download_budget = download_budget;
    wave_future.emplace(std::async(
        std::launch::async, [child = std::move(child), unbounded_http_get, now,
                             progress]() mutable {
          return GenerateEnvironment(child, unbounded_http_get, now, progress);
        }));
  }
  ...
  if (restored_weather) {
    ...
    const auto weather =
        request.weather_provider == "dwd_icon_eu"
            ...
            : GenerateHrrr(
                  ...
                  MakeRetryingHttpGetRange(http_get_range, provider, progress),
                  now, progress);
    streams.emplace_back("weather", weather.output);
    selected_cycle = weather.cycle.CycleTime();
    ...
    const auto weather = GenerateEcmwfOpenData(
        ...
        MakeRetryingHttpGetRange(http_get_range, provider, progress), now,
        progress);
    streams.emplace_back("weather", weather.output);
    selected_cycle = weather.cycle.CycleTime();
  ...
  if (current_future) {
    ...
    if (current_source == "copernicus_global")
      generated =
          GenerateCopernicusGlobal(current, download, credential_validator);
    else if (current_source == "copernicus_ibi")
      generated =
          GenerateCopernicusIbi(current, download, credential_validator);
    else if (current_source == "copernicus_mediterranean")
      generated = GenerateCopernicusMediterranean(current, download,
                                                  credential_validator);
    else
      generated =
          GenerateCopernicusNws(current, download, credential_validator);
    streams.emplace_back("current", generated.output);
    ...
    current.grid_spacing_deg = request.current_grid_spacing_deg;
    current.overwrite = true;
    TextDownload rtofs_text = [http_get](const std::string& url,
                                         double timeout) {
      const auto bytes = http_get(url, timeout);
      return std::string(bytes.begin(), bytes.end());
    };
    streams.emplace_back(
        "current", GenerateRtofs(current, std::move(rtofs_text),
                                 BinaryDownload(MakeRetryingHttpGet(
