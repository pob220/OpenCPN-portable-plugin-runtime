...
int main() {
  ...
  job_request["maxConcurrentDownloads"] = 3;
  ...
  Check(parsed_job.request.bbox.west == -8.5 &&
            ...
            parsed_job.request.max_concurrent_downloads == 3,
  ...
  Check(capabilities["schemaVersion"].asInt() == 1 &&
            ...
            capabilities["defaultMaxConcurrentDownloads"].asInt() == 4 &&
            capabilities["maximumConcurrentDownloads"].asInt() == 8,
        "job protocol capabilities");
  job_request["maxConcurrentDownloads"] = 0;
  bool invalid_download_bound = false;
  try {
    eg::ParseGeneratorJob(job_json);
  } catch (const eg::ValidationError&) {
    invalid_download_bound = true;
  }
  Check(invalid_download_bound,
        "job protocol rejects an invalid download concurrency bound");
  job_request["maxConcurrentDownloads"] = 3;
