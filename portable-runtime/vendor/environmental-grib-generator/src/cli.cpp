...
int GenerateEnvironment(const std::vector<std::string>& args) {
  ...
  for (std::size_t i = 1; i < args.size(); ++i) {
    ...
    else if (arg == "--max-concurrent-downloads")
      request.max_concurrent_downloads =
          static_cast<std::size_t>(ParseInt(RequireValue(args, i, arg), arg));
    else if (arg == "--dry-run")
      request.dry_run = true;
...
int Normalize(const std::vector<std::string>& args) {
  ...
  const auto result = eg::NormalizeGribStream(eg::PathFromUtf8(args[1]),
                                              eg::PathFromUtf8(args[2]));
  Json::Value json(Json::objectValue);
  json["message_count"] = Json::UInt64(result.message_count);
