...
GeneratorJob ParseGeneratorJob(const Json::Value& value) {
  ...
  job.request.max_concurrent_downloads = static_cast<std::size_t>(
      Integer(request, "maxConcurrentDownloads", 4));
  ...
  if (job.request.max_concurrent_downloads == 0 ||
      job.request.max_concurrent_downloads > 8) {
    throw ValidationError(
        "maxConcurrentDownloads must be between 1 and 8");
  }
  return job;
}
...
Json::Value GeneratorCapabilitiesJson() {
  ...
  value["defaultMaxConcurrentDownloads"] = 4;
  value["maximumConcurrentDownloads"] = 8;
  return value;
}
