...
int32_t PortablePluginManager::Impl::OpenWeatherRouting(void* user_data) {
  ...
  if (!instance->weather_routing_host) {
    ...
    auto routes = [owner = instance->owner, instance]() {
      ...
      for (size_t index = 0;
           index < ids.size() && result.size() < kNavigationObjectLimit;
           ++index) {
        auto route = GetRoute_Plugin(ids[index]);
        if (!route || route->m_GUID.empty() || !route->pWaypointList)
          continue;
        PortableNavigationRoute snapshot;
        snapshot.id = route->m_GUID;
        snapshot.name = route->m_NameString.empty() ? "Unnamed route"
                                                    : route->m_NameString;
        for (auto node = route->pWaypointList->GetFirst(); node;
             node = node->GetNext()) {
          const auto* waypoint = node->GetData();
          if (!waypoint || !std::isfinite(waypoint->m_lat) ||
            ...
            continue;
          wxString name = waypoint->m_MarkName;
          if (name.empty()) name = "Route waypoint";
          snapshot.points.push_back({waypoint->m_GUID, name, waypoint->m_lat,
                                     waypoint->m_lon});
          if (snapshot.points.size() >= kNavigationObjectLimit) break;
        }
        if (snapshot.points.size() >= 2) result.push_back(std::move(snapshot));
      }
      std::stable_sort(result.begin(), result.end(),
                       [](const auto& left, const auto& right) {
                         return left.name.CmpNoCase(right.name) < 0;
    ...
    auto create_route = [owner = instance->owner, instance](
                            const wxString& name,
                            const std::vector<PortableNavigationPosition>& points,
                            wxString* error) {
      if (!owner->HasPermission(*instance, "navigation.routes.write")) {
        if (error) *error = "navigation.routes.write permission was not granted";
        return false;
      }
      if (points.size() < 2 || points.size() > kNavigationObjectLimit) {
        if (error) *error = "route must contain between 2 and 10000 points";
        return false;
      }
      PlugIn_Route route;
      route.m_NameString = name.empty() ? "Portable routing passage" : name;
      route.m_StartString = points.front().name;
      route.m_EndString = points.back().name;
      std::vector<std::unique_ptr<PlugIn_Waypoint>> owned_points;
      owned_points.reserve(points.size());
      for (size_t index = 0; index < points.size(); ++index) {
        const auto& point = points[index];
        if (!std::isfinite(point.latitude) ||
            !std::isfinite(point.longitude) || point.latitude < -90.0 ||
            point.latitude > 90.0 || point.longitude < -180.0 ||
            point.longitude > 180.0) {
          if (error) *error = "route contains an invalid position";
          return false;
        }
        auto waypoint = std::make_unique<PlugIn_Waypoint>(
            point.latitude, point.longitude, "diamond",
            index == 0 ? "Route start"
            : index + 1 == points.size() ? "Route destination"
                                         : wxString::Format("Route point %zu", index));
        waypoint->m_MarkDescription =
            "Planning output generated by a portable routing component";
        route.pWaypointList->Append(waypoint.get());
        owned_points.push_back(std::move(waypoint));
      }
      if (!AddPlugInRoute(&route, true)) {
        if (error) *error = "OpenCPN rejected the generated route";
        return false;
      }
      return true;
    };
    ...
    instance->weather_routing_host =
        std::make_unique<PortableWeatherRoutingHost>(
            ...
            instance->id, instance->routing_surface,
            std::move(summary), std::move(waypoints), std::move(routes),
...
void PortablePluginManager::Impl::Fail(Instance& instance,
  ...
  if (instance.environmental_host) instance.environmental_host->RequestStop();
...
bool PortablePluginManager::Impl::Load() {
  ...
  while (more) {
    ...
    if (manifest["surfaces"].IsObject()) {
      ...
      const wxString routing_surface =
          manifest["surfaces"]["routing.workbench"].AsString();
      if (!routing_surface.empty() && IsSafeRelativePath(routing_surface))
        instance->routing_surface = routing_surface;
    }
...
void PortablePluginManager::Impl::Shutdown() {
  ...
  stopped = true;
  alive->store(false);
  // Signal provider-side helper work before joining consumer workers.  A
  // routing worker may currently be blocked in an environmental batch call.
  for (auto& instance : instances) {
    if (instance->environmental_host)
      instance->environmental_host->RequestStop();
  }
  for (auto& instance : instances) {
    StopJobs(*instance);
    if (instance->weather_routing_host)
