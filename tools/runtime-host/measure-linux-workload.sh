...
Runs the installed-package Holyhead-to-Dun-Laoghaire route probe twice and
...
the bundled cruising polar, then shuts the runtime down after each run.
...
if [[ ! -x "${probe}" || ! -f "${fixture}" || ! -f "${polar}" ]]; then
  printf 'The route probe, GRIB fixture or polar is missing.\n' >&2
  exit 1
fi
...
for run in $(seq 1 "${runs}"); do
  ...
  start_ns="$(date +%s%N)"
  stdbuf -oL -eL "${probe}" "${store}" "${fixture}" "${polar}" \
    1784800800 53.336985 -4.607470 53.308408 -6.119702 \
    >"${output}" 2>&1 &
  probe_pid="$!"
  peak_rss=0
  ...
  if ! grep -Fq 'route=complete ' "${output}" ||
     ! grep -Fq 'Reverse-isocrone recovery' "${output}" ||
